        ...
        <a class="text-dark text-decoration-none" href="{{ route('listings.show', ['id' => $upload->id]) }}"
            ...
            <div class="card listing-card my-3" style="min-width: 20rem !important;">
                <img class="relative card-img"
                    src="{{ route('images', ['path' => $upload->images, 'number' => 0]) }}"
                ...
                <div class="position-absolute mt-2 ml-2">
                    <span class="badge badge-success">
                        ...
                        @elseif ($updated_at->diffInMinutes() < 60) {{ __('New') }} @elseif ($updated_at->
                            diffInHours() < 24) {{ __('New') }} - {{ __($updated_at->diffForHumans()) }}
                        @else 
